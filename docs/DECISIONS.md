...
| DEC-103 | Complementos do FP Food devem ser modelados como grupos vinculados ao produto principal, com opcoes selecionadas por padrao quando representarem ingredientes inclusos. A primeira fase deve focar em remocoes sem preco; adicionais pagos, limites avancados, combos e regras promocionais entram depois. O carrinho, a revalidacao e o pedido devem guardar snapshot da composicao escolhida por linha de item. | Aprovada |
| DEC-104 | A operacao interna do FP Food deve separar Atendimento de Pedidos: Atendimento concentra criacao de pedido de balcao, telefone e futura mesa/comanda; Pedidos concentra acompanhamento, filtros, status, detalhe e acoes operacionais. Controle de mesa deve evoluir como sessao/comanda de mesa, permitindo itens incrementais antes do fechamento e pagamento. | Aprovada |
| DEC-105 | O pedido do FP Food permanece agrupado para cliente, cobranca, fiscal e totalizacao, mas a operacao de cozinha deve controlar preparo por item em `food.order_items.item_status`. Itens novos adicionados a pedido de balcao ainda pendente de pagamento entram como nova demanda de cozinha quando exigem preparo, sem reabrir itens ja prontos. Lotes/comandas de producao ficam como evolucao posterior para mesas, complementos incrementais e auditoria operacional mais rica. | Aprovada |
| DEC-106 | Produtos do FP Food com controle de estoque ativo devem impedir pedido quando o saldo disponivel for insuficiente ou zero. A criacao do pedido baixa o estoque automaticamente por delta de venda; correcoes antes do pagamento ajustam apenas a diferenca; cancelamentos devolvem o saldo por ajuste. As movimentacoes devem ficar registradas em `food.stock_movements` para rastreabilidade operacional. | Aprovada |
| DEC-107 | Webhooks Mercado Pago em producao (`live_mode=true`) devem manter validacao rigida de assinatura. Em sandbox (`live_mode=false`), quando o Mercado Pago assinar eventos reais com a aplicacao das credenciais de teste enquanto o simulador usa a aplicacao geral, o Gateway pode aceitar a notificacao apenas como gatilho, consultar a Order no Mercado Pago com o Access Token da empresa e conciliar somente com a resposta oficial do provedor. | Aprovada |
| DEC-108 | A observabilidade futura do webhook Mercado Pago deve reduzir ruido em sandbox: o evento `sandbox_signature_fallback` deve permanecer rastreavel, mas os logs diagnosticos de assinatura invalida podem ser rebaixados/agrupados quando o fallback sandbox resultar em conciliacao oficial bem-sucedida. | Aprovada |
| DEC-109 | A configuracao Mercado Pago do Gateway deve evoluir para validar e exibir o App ID esperado por ambiente/credencial, alertando quando o `application_id` recebido no webhook divergir do provedor configurado para a empresa. | Aprovada |
| DEC-110 | Mesa/comanda no FP Food deve nascer como sessao operacional antes do pedido fiscal/comercial final: `food.dining_tables` representa a mesa, `food.table_sessions` representa a comanda aberta e os itens podem ser adicionados incrementalmente ate o fechamento. O pagamento da fase inicial deve ser unico no fechamento da comanda; pagamentos parciais ficam para evolucao posterior. | Aprovada |
| DEC-111 | Pedido em lote para cozinha deve representar lote de producao, nao novo pedido comercial. O pedido/comanda continua sendo a unidade de cliente, totalizacao, pagamento e fiscal; cada envio incremental de itens de cozinha pode gerar um lote imprimivel com seus proprios itens, horario, origem e status, sem reabrir lotes ou itens ja prontos. | Aprovada |
